...
class ExecShell(test_case.TestCase):
  ...
  ARGS = [
      ...
      Arg('autostart', bool,
          'True auto Start, False Press Space to Start.', default=True),
      Arg('tipMsg', str,
          ('Msg for not autostart.'), default='Please press space to start testing.'),
  ]
  ...
  def runTest(self):
    ...
    if not self.args.autostart:
      self.ui.SetInstruction(_(self.args.tipMsg))
