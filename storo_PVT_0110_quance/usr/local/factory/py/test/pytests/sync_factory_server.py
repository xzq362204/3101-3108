...
class SyncFactoryServer(test_case.TestCase):
  ARGS = [
      ...
      Arg('update_without_prompt', bool, 'Update without prompting when an '
          'update is available.', default=True),
      ...
      Arg('server_url', (str, dict),
          ...
          default=None),
      Arg('loop_count_ping', int, 'ping loop count. 0:not limit, >1 max retry times', default=0)
  ]
  ...
  def runTest(self):
    ...
    for label, task in tasks:
      if task == self.Ping and self.args.loop_count_ping > 0:
        device_data.UpdateDeviceData({'factory.ping_fail': False})
      while True:
        try:
          logging.info('Running task: %s.', label['en-US'])
          self.ui.SetState(_('Running task: {label}', label=label))
          if task == self.Ping and self.args.loop_count_ping>0:
              loop_count_ping = loop_count_ping + 1
          task()
        ...
        if task == self.Ping and self.args.loop_count_ping>0 and loop_count_ping>self.args.loop_count_ping:
            device_data.UpdateDeviceData({'factory.ping_fail': True})
            self.FailTask("Ping Failed")
