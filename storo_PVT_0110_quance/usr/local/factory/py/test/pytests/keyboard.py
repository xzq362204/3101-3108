...
class KeyboardTest(test_case.TestCase):
  ...
  def GetKeyboardLayout(self):
    ...
    modelchk = self.dut.CallOutput('factory device-data -g component.has_rear_camera').strip()
    if str(modelchk) == 'True':
      add_model_string = 'B'
    else:
      add_model_string =''
    print(add_model_string + regions.REGIONS[region].keyboard_mechanical_layout)
    return add_model_string+regions.REGIONS[region].keyboard_mechanical_layout
  ...
  def ReadKeyOrder(self, layout):
    ...
    key_order_list_filename = os.path.join(self.ui.GetStaticDirectoryPath(),
                                           layout + '.key_order')
    print(ast.literal_eval(file_utils.ReadFile(key_order_list_filename)))                                     
