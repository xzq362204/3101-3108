...
#    "${DISPLAY_MESSAGE}" "remove_ac"
    display_boot_message show_file ./factory/sh/cutoff/qr_flow.png
...
is_usb_present() {
  ls /sys/bus/usb/devices | grep 1-1:1.0 ||\
  ls /sys/bus/usb/devices | grep 1-1:1.1
}

is_usbhid() {
  ls /sys/bus/usb/devices/1-1:1.0/driver/module/drivers | grep usb:usbhid || \
  ls /sys/bus/usb/devices/1-1:1.1/driver/module/drivers | grep usb:usbhid
}
...
check_battery_value() {
  ...
  if [ -n "${max_battery_value}" ] &&
    ...
    while [ "${battery_value}" -gt "${max_battery_value}" ]; do
      ...
      if [ "${battery_value}" -ne "${prev_battery_value}" ]; then
        # Keep printing battery information in the same line.
        if [ "${battery_value}" -gt "${prev_battery_value}" ]; then
          charge_control "discharge"
        fi
        printf '\rcurrent: %s, target: %s' \
...
main() {
  ...
  CUTOFF_BATTERY_MIN_PERCENTAGE_1="60"
  CUTOFF_BATTERY_MAX_PERCENTAGE_1="80"
  ...
  if [ -n "${battery_path}" ]; then
    ...
    mkdir -p /var/lib/power_manager
    modprobe i2c_dev || true
    if [ -n "${CUTOFF_BATTERY_MIN_PERCENTAGE_1}" ] ||
       [ -n "${CUTOFF_BATTERY_MAX_PERCENTAGE_1}" ]; then
      check_battery_value \
        "${CUTOFF_BATTERY_MIN_PERCENTAGE_1}" "${CUTOFF_BATTERY_MAX_PERCENTAGE_1}" \
        "get_ectool_battery_percentage" "" ||
      check_battery_value \
        "${CUTOFF_BATTERY_MIN_PERCENTAGE_1}" "${CUTOFF_BATTERY_MAX_PERCENTAGE_1}" \
        "get_battery_percentage" "${battery_path}" ||
...
# Secondary control of current
    # Ask operator to plug or unplug AC before doing cut off.
    # The operator might not do this immediately, so we set the charge status to
    # idle to keep the charge percentage stable, and set back to normal just
    # before doing cutting off.
    charge_control "idle"
    check_ac_state1 "${CUTOFF_AC_STATE}"
    charge_control "normal"
  else
    echo "Battery not found."
  fi
