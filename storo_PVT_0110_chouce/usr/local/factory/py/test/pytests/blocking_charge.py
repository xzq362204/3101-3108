...
class ChargerTest(test_case.TestCase):
  ARGS = [
      Arg('target_charge_pct', (int, type_utils.Enum(['goofy', 'cutoff','mainlist'])),
      ...
      Arg('dim_backlight_pct', float,
          ...
          default=3.0),
      Arg('min_charge_pct', int, 'target charge min level',
          default=78),
      Arg('max_charge_pct', int, 'target charge max level',
          default=79),
  ]
  ...
  def runTest(self):
    ...
    elif target_charge == 'mainlist':
      target_charge = self.args.min_charge_pct
